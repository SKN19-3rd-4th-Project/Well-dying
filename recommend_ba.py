...
def search_empathy_questions_tool(
    ...
    keywords = []
    if recent_messages:
        stop = {"그리고", "그래서", "합니다", "했습니다", "그런데", "지금", "조금", "정말", "뭔가", "나는", "제가", "저는"}
        merged = " ".join(recent_messages[-5:])
        for token in merged.replace("\n", " ").split(" "):
            token = token.strip().strip(",.?!\"'()[]")
            if len(token) >= 2 and token not in stop:
                keywords.append(token)
        keywords = list(dict.fromkeys(keywords))[:5]  # dedup, top 5
